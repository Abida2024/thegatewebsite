...
class Scraper:
        ...
        for i in range(1, 27):
            ...
            for article in articles_soup:
                ...
                if self.scrape_insert_article(article_soup, article_url):
                    images = article_soup.select('div img')
                    for image in images:
                        if 'jpg' in image['src']:
                            img_url = image['src']
                            img_data = requests.get(img_url).content
                            name = self.url_im(article_soup, article_url)
                            name = name.split("/")[3]
                            f = '/Users/Abby 1/cmsc-uchicago/thegatewebsite/web-scraping/images/{name}.jpg'.format(name=name)
    ...
    def url_im(self, soup, article_url):
        '''' Scrape content and relevant metadata of an article 
        and insert into the database ''' 
        # Retrieve the relevant content: title, contributor, 
        # category, image_url, featured_image_caption, lede, body
...
def scrape_image(article_soup, article_url, img_url):
    images = article_soup.select('div img')
    for image in images:
        if 'jpg' in image['src']:
            img_url = image['src']
            img_data = requests.get(img_url).content
            name = img_url.split("/")[3]
            file =  "/Users/Abby 1/cmsc-uchicago/thegatewebsite/web-scraping/images/{name}".format(name=name)
